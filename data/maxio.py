...
import numpy as np
import torch
# from .atk_vocab_prep import get_pri_vocab
...
class DependencyInstance(object):
    def __init__(self, sentence, postags, pos_ids, heads, types, type_ids): #, is_pri, pri_word_ids):
        ...
        # self.is_pri = is_pri
        # self.pri_word_ids = pri_word_ids
...
class CoNLLXReader(object):
    def __init__(self, file_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet):
        ...
        # self.__pri_vocab = get_pri_vocab()
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False):
        ...
        for tokens in lines:
            ...
            type_ids.append(self.__type_alphabet.get_index(relation))

            heads.append(head)

            # is_pri.append(is_privacy_term(tokens[4]))
            # pri_word_ids.append(self.__pri_vocab.get_token_index(tokens[1], "pri_tokens"))
        ...
        return DependencyInstance(Sentence(words, word_ids, char_seqs, char_id_seqs), postags, pos_ids, heads, types, type_ids) # , is_pri, pri_word_ids)
