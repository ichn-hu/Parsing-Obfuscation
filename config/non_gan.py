...
class Global(MrDict):
    ...
    model = "non_gan"
    # model = "investigate_non_gan"
    exp_name = os.environ.get("exp_name", "seperated_model")
    ...
    gen_loss_term = ["loss_arc", "loss_rel", "loss_cpy", "loss_full_cpy", "loss_ent"]
    ...
    resume_result = os.path.join("$ROOT/Project/data/result", os.environ.get("resume_result", "random_obf:all_pri:NNP_rate:1.0-01.04_03:30-0.6243-0.1376"))
...
AlltagCopyCtxGeneratorCfg = SeqLabelGeneratorCfg.update({
    "cpy_penalty": float(os.environ.get("cpy_penalty", 0.0))
    })
