...
def MaskedRecurrent(reverse=False): # 每一层的操作
        ...
        for i in steps:
            if mask is None or mask[i].data.min() > 0.5: # 没有mask，都是1
                ...
                if isinstance(hidden, tuple): # LSTM outputs a tuple of (hidden, cell), this is a common hack 😁
                    ...
                    # import pdb
                    # pdb.set_trace()
                    hidden = (hx + (hp1 - hx) * mask[i].squeeze(), cx + (cp1 - cx) * mask[i].squeeze()) # Why? 我知道了！！如果是mask就不用改变
                else:
...
class MaskedRNNBase(nn.Module):
    ...
    def forward(self, input, mask=None, hx=None):
        ...
        output, hidden = func(input, self.all_cells, hx, None if mask is None else mask.view(mask.size() + (1, ))) # 这个+ (1, )是个什么操作？
